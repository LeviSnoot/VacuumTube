...
// Something's intercepting and preventing scrolling,
// so I made a custom scrolling solution.

// Track scroll offsets for each scrollable viewport by ID
const scrollOffsets = {}
    ...
    return el('div', {
        ...
        el('div', {
            ...
            el('div', { className: 'vt-settings-header' }, [
                el('span', { className: 'vt-settings-title', textContent: 'VacuumTube Settings' }),
                el('span', { className: 'vt-settings-hint', textContent: locale.settings_overlay.navigation_hint }),
                el('div', { className: 'vt-settings-close', dataAction: 'close' }, [
                    el('span', { textContent: '✕' })
                ])
            ]),
            el('div', { className: 'vt-settings-body' }, [
                el('div', { className: 'vt-settings-tabs' }, [
                    createTab('adblock', locale.settings.ad_block.title, 0, true),
                    createTab('userstyles', locale.settings.userstyles.title, 1, false)
                ]),
                el('div', { className: 'vt-settings-content' }, [
                    ...
                    el('div', { className: 'vt-content-panel', dataPanel: 'userstyles' }, [
                        el('div', { className: 'vt-userstyles-section' }, [
                            el('p', { className: 'vt-userstyles-description', textContent: locale.settings_overlay.userstyles.description }),
                            // Global userstyles toggle
                            el('div', {
                                ...
                                el('div', { className: 'vt-setting-info' }, [
                                    el('span', { className: 'vt-setting-title', textContent: locale.settings_overlay.userstyles.enable_all })
                                ]),
                                el('div', { className: 'vt-setting-control' }, [
                            ...
                            el('div', {
                                ...
                                el('span', { textContent: locale.settings_overlay.userstyles.open_folder })
                            ])
                        ])
                    ])
                ])
...
function updateFocus(area) {
    ...
    if (area === 'tabs') {
        ...
        if (tab) tab.classList.add('vt-tab-focused')
        ...
        if (panel) {
            ...
            if (focusedElement && currentTabIndex === 1) {
